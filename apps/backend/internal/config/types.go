...
type Config struct {
	...
	Redis     RedisConfig     `env:",squash"`
	AI        AIConfig        `env:",squash"`
	Midtrans  MidtransConfig  `env:",squash"`
}
...
type AIConfig struct {
	...
	DefaultModel     string `env:"AI_DEFAULT_MODEL"`
	AllowedModels    string `env:"AI_ALLOWED_MODELS"` // Comma-separated
	ModeRouting      string `env:"AI_MODE_ROUTING"`    // Format: "fast:model1|accurate:model2|knowledge:model3"
}

type MidtransConfig struct {
	ServerKey string `env:"MIDTRANS_SERVER_KEY"`
	ClientKey string `env:"MIDTRANS_CLIENT_KEY"`
	Env       string `env:"MIDTRANS_ENV"` // sandbox or production
}
