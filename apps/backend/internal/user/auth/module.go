...
import (
	...
	"github.com/detect-price-by-photo/backend/internal/user/auth/handler"
	...
	authsvc "github.com/detect-price-by-photo/backend/internal/user/auth/services"
...
func NewModule(opts *Options) *AuthModule {
	...
	authService := authsvc.NewAuthService(authsvc.AuthServiceOpts{
		AuthRepo:           authRepo,
...
func (m *AuthModule) RegisterRoutes(e *echo.Group) {
	...
	publicGroup.POST("/forgot-password", m.handler.ForgotPassword)
	publicGroup.POST("/reset-password", m.handler.ResetPassword)
